...
public class RedGridPaperView extends View {
    ...
    private static final float CELL_PADDING_RATIO = 0.15f; // 文字内边距比例
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // 高度 = (行数 * 格子高度) + ((行数 - 1) * 行间距) + 底部留白（为字数标注留出空间）
        int desiredHeight = (int) (rowCount * cellSize + (rowCount - 1) * ROW_SPACING + MARK_TEXT_SIZE + 20);
    ...
    private void drawGrid(Canvas canvas) {
        ...
        for (int row = 0; row < rowCount; row++) {
            float rowStartY = row * (cellSize + ROW_SPACING) + BORDER_WIDTH / 2f;
    ...
    private void drawBorder(Canvas canvas) {
        ...
        // 绘制每一行的独立边框
        for (int row = 0; row < rowCount; row++) {
            float rowStartY = row * (cellSize + ROW_SPACING);
            float rowEndY = rowStartY + cellSize;
    ...
    private void drawText(Canvas canvas) {
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            float y = i * (cellSize + ROW_SPACING) + textOffset;
    ...
    private void drawWordCountMarks(Canvas canvas) {
        ...
        for (int row = 3; row < rowCount; row += 4) { // row 4 是第5行（从0开始计数）
            ...
            // 计算标注的Y位置：在当前行的下方，紧挨着格子底部
            float rowBottomY = (row + 1) * (cellSize + ROW_SPACING); // 当前行底部位置
